...
- [S#e7f3a2b1] **Unfilled positions, when unavoidable, should be concentrated on the least clinically-critical and least desirable shifts** — protecting skill-required coverage first, then Sunday, then Saturday, then weekday day-shifts, leaving weekday night General shifts as the first to go unfilled. Since casual staff are now sourced entirely outside this system, `UNFILLED` is the expected, routine way this system reports a structural staffing gap. Rather than treating every unfilled position equally, the model uses a tiered penalty system where each tier corresponds to a position's "undesirability" (combining skill criticality, day of week, and shift timing). Every tier's weight exceeds the worst-case combined soft-constraint penalty, preserving the safety guarantee that the solver never leaves a position unfilled just to save a soft-constraint penalty.
  ...
  - **Tier 2 (Sunday, General)** — Sunday wildcard positions (all shift types). Weight: 210000. Sunday has 200% loading — the most desirable shift.
  - **Tier 3 (Saturday, General)** — Saturday wildcard positions (all shift types). Weight: 200000. Saturday has 150% loading — desirable.
  - **Tier 4 (weekday day, General)** — weekday D8/D12/P8/P12/L3/DISCO wildcard positions. Weight: 160000. Neutral desirability.
  - **Tier 5 (weekday night, General)** — weekday N8/N12 wildcard positions. Weight: 140000. Least desirable shift — first to go unfilled.

  The solver uses the tier weight as the unfilled penalty for each position. Higher penalty = less likely to be left unfilled. Weekend day/night shifts fall under the Saturday/Sunday tiers (higher protection than weekday shifts).

## Fairness & Distribution (Weekend)
- ~~[S#a1d6c3d5]~~ ~~Weekend hours fairness (blended Saturday + Sunday).~~ **Replaced by separate Saturday and Sunday fairness constraints** ([S#s1a2t3u4] and [S#s2u3n4d5]) so that Saturday and Sunday hours are distributed independently — a staff member cannot satisfy their "fair share" of weekend hours entirely via Saturday and never get a Sunday shift (or vice versa).
- [S#s1a2t3u4] **Saturday hours fairness.** Minimizes the sum of absolute deviations of each staff member's Saturday hours from the mean. Saturday has 150% loading and should be distributed fairly across staff.
- [S#s2u3n4d5] **Sunday hours fairness.** Minimizes the sum of absolute deviations of each staff member's Sunday hours from the mean. Sunday has 200% loading — the most valuable shift day — and should be distributed fairly across staff.
