...
| **✅ Fully** | `[H#5e6ad8f4]` | Skill level matching (threshold) | `SkillLevelRequirement` | CP-SAT reified constraint: `model.Add(staff_rank >= required_rank).OnlyEnforceIf(x)`. Covers both [H#5e6ad8f4] and [H#84a1d5c9]. |
...
| **✅ Fully** | `[H#84a1d5c9]` | Skill level hierarchy (higher satisfies lower) | `SkillLevelHierarchy` | No-op at CP-SAT level. Merged into `SkillLevelRequirement` which enforces threshold semantics. Data validation ensures contiguous prefix. |
...
| **✅ Fully** | `[H#f0c5b2c4]` | 76h absolute paid-hour cap per block | `MaxHoursConstraint` | Enforced via IntVar upper bound of `76*SCALE` in `solver._create_variables()`. The `apply()` is a no-op — the bound is set at variable creation. `ContractedHoursFloor` ([H#d9a8b7c6]) handles the lower bound. |
...
- **2026-08-05**: Implemented `[H#5e6ad8f4]` (skill level matching) and `[H#84a1d5c9]` (skill level hierarchy) — `SkillLevelRequirement` class now enforces `model.Add(staff_rank >= required_rank).OnlyEnforceIf(x)` per staff-position pair with non-null requirements. `SkillLevelHierarchy` merged into `SkillLevelRequirement` as a no-op toggle class (hierarchy validated at data-loading time, threshold check done at solve time). `[H#f0c5b2c4]` (76h cap) confirmed as no-op (enforced via IntVar bounds in `_create_variables()`). Added 9 tests (6 for SkillLevelRequirement, 3 for MaxHoursConstraint). All 145 tests pass. Roster generation: OPTIMAL, 404 assignments, 0 casual, 0 unfilled.
