...
- [H#f4c9b6c8] Staff must have at least 1 full day off between a night shift and a day shift (in either direction). This is mandatory, not a preference — do not treat the "should" wording as optional.
...
- [H#d9a8b7c6] Staff must be rostered on for at least their contracted **paid** hours per fortnight (per `definitions.yaml`'s `paid_hours` field, not `span_hours`; adjusted proportionally for **holidays only** — red requests do not reduce the floor, since they are single unavailable days rather than extended leave) per 14-day fortnight block. This is enforced on a per-block basis and cannot be averaged across the entire roster period.
- [H#e8f7d6c5] Staff may not work more than 24 additional **paid** hours (per `definitions.yaml`'s `paid_hours` field) beyond their contracted hours in any 14-day fortnight block.
